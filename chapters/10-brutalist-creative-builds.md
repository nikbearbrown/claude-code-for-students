...
1. **(Understand)** Explain how the fluency trap manifests in creative AI use — why polished output can be authorless.
2. **(Apply)** Apply the Brutalist three-file system (CLAUDE.md, DESIGN.md, PROJECT.md) to a creative project.
3. **(Analyze)** Distinguish creative judgment (irreducibly human, currently) from technical execution (Claude's domain) in a provided creative build.

---

## Opening: The Void Underneath the Polish

Nicholas works on the creative side of this book. He is the contributor who watches what happens in the studios and the design labs and the music classes that the AP-CS frame of Chapters 0 through 9 doesn't quite reach. His observation is the one this chapter exists to honor.
...
The argument of this chapter is that the same discipline that solved the code problem also solves the creative one. The discipline has a name. It comes from architecture by way of conceptual art and lands, in 2026, on a framework Nicholas calls **Brutalist**.[^brutalist-site] Brutalist is the operational form of *draw the line* for creative builds. It is what you write down before Claude touches the file, so that when Claude touches the file Claude does the technical work and you keep the creative judgment. The previous nine chapters drew the line for code. This one draws it for everything else.

[^brutalist-site]: The live framework documentation is at [brutalist.art](https://brutalist.art). The site is the canonical reference for the exact file schemas, the refusal-behavior spec, and the running list of student creative builds in the wild. The framework name borrows deliberately from architectural Brutalism, on which see footnote 4.

<!-- → [DIAGRAM: The three-file system as three nested layers. Outer: CLAUDE.md (technical constitution — what Claude never improvises). Middle: DESIGN.md (visual constitution — every aesthetic decision specified or escalated). Inner: PROJECT.md (project state — Intent Layer is human, always). Editorial.] -->

The diagram above is the chapter in one image. Three nested constitutions, increasingly close to the work, increasingly close to *you*. The outer ring is technical — the stack, the dependencies, the file structure, the things Claude must never improvise about. The middle ring is aesthetic — the palette, the type, the spacing, every decision about how the work looks. The inner ring is the project itself — what it is, where it stands, and at its center, in a box marked *INTENT LAYER — HUMAN*, the thing only a person can supply: what the work is *for*, what it is supposed to feel like, what the maker is trying to say.

The discipline of Brutalist is to keep the human box human. Everything outside it can be delegated. The thing inside it cannot.

## The Fluency Trap in Creative Work
...
The empirical record is now strong enough to settle the question. The cleanest single data point is the *Frontiers in Education* 2025 synthesis on AI in academic writing: across post-ChatGPT college admissions essay corpora, lexical diversity *rose* — students used more varied vocabulary — while thematic distinctiveness across essays *fell*.[^fie-2025] That is the polish-and-void pattern in one number. The essays got smoother and the essays got more identical. A reader picking up any one of them would have said *this is well written*. A reader picking up a hundred of them would have said *I have read this exact essay before*.
...
Brutalist makes six commitments. They are not arbitrary; each one names a specific thing the framework refuses to let Claude do. The names below are the chapter's articulation; the live framework page may refine them.
...
**Two: Refusal over guessing.** When the spec is ambiguous, Claude refuses to proceed and surfaces the ambiguity. The refusal is not a bug. The refusal is the entire point. A model that guesses smoothly is a model that erodes authorship one micro-decision at a time. A model that refuses to guess is a model that *forces the human to remain in the decision*.

**Three: Materials as found.** No hidden dependencies. No "I'll pull a quick font from the web." No "I'll regenerate the data because the version on disk looked stale." The materials the project uses are the materials in the project's directories, named in CLAUDE.md, version-pinned. If the build runs, the build runs *on what is actually in the building*.
...
Claude refuses when **the Intent Layer is empty or missing**. PROJECT.md without the five questions answered is a project Claude will not begin. This is unfamiliar. It is closer to a co-author saying *I cannot write this until you tell me what it is for* than to a tool saying *processing your request*. The unfamiliarity is the point. Students conditioned by tools that produce something on every prompt will, on first encounter, read the refusal as a tool malfunction. It is not. It is the tool doing the one thing the prevailing tools refuse to do: insist that the human be present before the work begins.

Claude refuses when **its proposed output would change a file outside the declared scope**. Scope is named per session — *this session may modify `charts/`; everything else is read-only*. If Claude proposes an edit to a file outside the scope, Claude does not execute. Claude surfaces the proposal and waits. This is the creative-build analog of the negative constraint from Chapter 8 and the scope statement from Chapter 9. Agentic drift is a creative-work failure mode as much as a code one; the chart-rendering session that "while it was at it" rewrote the dataset is the same class of failure as the parser fix that "while it was at it" renamed a model field.
...
<!-- → [TABLE: Labor separation in creative builds — two columns. AI handles / Human keeps. Examples from visual, writing, and music domains. No color.] -->

The table above is the chapter's working operational summary. Read down the right column and you see what stays with the student. Notice that the right column entries are not skills the student has to *acquire* — they are decisions the student already has to make, that the prevailing workflow lets the model make instead. Brutalist does not add work. It returns work that was already the student's.

## Nicholas's Principle
...
[^benjamin]: Benjamin, Walter (1935). "The Work of Art in the Age of Mechanical Reproduction." First published in French in *Zeitschrift für Sozialforschung* V (1936); English translation by Harry Zohn in *Illuminations*, ed. Hannah Arendt (New York: Schocken Books, 1969). The *aura* passages — the unique presence of the work, the loss of authority under reproduction — appear in §II–IV of the standard 1969 translation.

## Worked Example: Seth's Dropout-Rate Chart, Built Twice

Seth had a chart to make. It was for the school newspaper. The data was the district's high-school dropout rate, year by year, 2010 through 2025. The point of the chart was to argue that the rate had *risen* in 2020–2022 — a pandemic spike — and then come down, but not all the way down. The newspaper editor wanted the chart in the print edition, which is black-and-white, and on the website, which is color.

Seth built the chart twice. The same Claude. The same data file. The same technical request: *render a line chart of dropout rate, 2010 to 2025, with annotations.* What differed was the framework around the request.

**Build One: prompt only.** Seth opened Claude and said: *render a clean line chart showing the high school dropout rate from 2010 to 2025; annotate the 2020 spike; make it look professional.* Claude produced a chart in about ninety seconds. Vega-Lite. The line teal-blue. The annotation arrow soft orange. The font Inter. Zero-based y-axis. Title bold and centered. The chart looked good.

Seth shipped it. The editor approved. It appeared in the paper. Two days later, in critique, the design teacher asked him why the spike was orange. Seth said *that's what came back*. She asked why the line was teal-blue instead of the school newspaper's masthead red. He said *that's what came back*. She asked why the y-axis started at zero instead of at the lowest observed value, which would have made the rise look more dramatic and more honest. He said *that's how Claude did it*. She told him, gently, that he had not made a chart. He had *received* a chart. Claude's defaults had decided it — viridis-adjacent palette, Inter, zero-based axis, soft orange for emphasis. The school newspaper's identity was nowhere in it. Worse: the chart's *argument* — the rise-and-incomplete-recovery — was muted by the zero-based axis, which made the spike look small. Seth had wanted the chart to argue that the rise had not been undone. As built, it argued the opposite: that the rise was a small bump in an otherwise steady series. Claude's defaults had inflected the meaning.

**Build Two: Brutalist.** Seth wrote three files first.

CLAUDE.md: *Project uses Vega-Lite. Data in `data/dropout_2010_2025.csv`, not regenerated. Outputs to `out/chart.svg` and `out/chart.png` at 300 DPI. No external font fetching. No animation.*

DESIGN.md: *Primary line color: `#B22222` (newspaper masthead red). Annotation color: `#1A1A1A` (black, for print compatibility). Background: white (`#FFFFFF`). Font: Source Serif Pro 12pt for axis labels, 14pt for title, 11pt for annotation. Y-axis: starts at the lowest observed value rounded down to nearest whole percentage point; not zero-based. Y-axis label: "Dropout Rate (%)" — exact string. X-axis: every two years labeled, gridlines at every year, light gray. Title: "The Spike That Didn't Fully Recede" — exact string. Annotation: arrow from text "Pandemic peak: 2021" pointing to the 2021 data point. No legend (single series). No chartjunk. Escalation list: any color not listed; any font; any axis behavior not specified.*

PROJECT.md INTENT LAYER: *This chart is for the November issue of the school newspaper. The reader is a student or parent. The argument is that the pandemic-era dropout spike has come down but has not returned to pre-2020 levels — there is residual harm. The reader should feel that the rise was real and the partial recovery is not enough. The chart explicitly refuses to argue that the situation is back to normal. The chart explicitly refuses to use the rhetorical move of starting the y-axis at zero, which would visually minimize the spike.*

Then Seth ran the same prompt: *render the chart per CLAUDE.md and DESIGN.md, honoring the PROJECT.md intent.*

Claude refused once: the DESIGN.md said *axis starts at the lowest observed value rounded down*, and Claude flagged that the lowest observed value in the dataset was 4.1% — should the axis start at 4%, or should it round further down to 3% for visual breathing room? Seth answered: 4%. Claude proceeded. The chart rendered in about ninety seconds, same as before. It was visually different. The line was red. The annotation pointed in black to 2021. The axis ran from 4% to 9%. The spike, at 8.4% in 2021, dominated the chart vertically; the partial recovery to 6.2% in 2025 was still visibly above the 2010–2019 baseline of around 4.5%. The argument the chart made was the argument the Intent Layer said it should make. The chart looked like the school newspaper, because the masthead red was in DESIGN.md. The chart was a print artifact, because the colors had been chosen for print compatibility, because CLAUDE.md said so.

In critique, the design teacher asked Seth why the line was red. He said: *masthead*. She asked why the axis started at four. He said: *because zero-based axes minimize the thing the chart is trying to argue, and the chart is arguing that the rise has not been undone*. She asked why the annotation was in black. He said: *print compatibility*. He answered three questions. He had a reason for each one. He had not made the chart with his hands — Claude had — but every decision in the chart was traceable to a decision he had made, in writing, before Claude saw the request.

Same Claude. Same data. Same render time. Different chart. Different authorship. The chart in Build Two is, in the documentary sense the Copyright Office cares about, Seth's. The chart in Build One was Claude's, with Seth's name attached. Brutalist is the procedure that produces the first kind of chart instead of the second.

## Exercises

1. **(Apply)** Write a DESIGN.md for a creative project you are actually working on — a chart, a poster, a slide deck, a zine page, a short film, a song. Name **six design decisions that are fully specified** (with values: hex codes, font names, sizes, exact strings) and **two design decisions that are escalated to you** (with the escalation trigger written out — what condition causes Claude to stop and ask). Show the DESIGN.md to a classmate. Ask: *is there any decision Claude could make that is not either specified or escalated?* If yes, the DESIGN.md is incomplete. Fix it.

2. **(Analyze)** Below is a transcript fragment from a creative build session in which Claude makes an aesthetic judgment. Identify the *exact moment* the judgment happens and name which of the three files (CLAUDE.md, DESIGN.md, PROJECT.md) would have prevented it. State, in one sentence, what line in that file should have existed.

   > **STUDENT:** Render the album cover. It should feel cold and lonely.
   >
   > **CLAUDE:** Done. I've used a deep blue gradient (`#0B1929` to `#1A3A5C`) with a centered serif title in Cormorant Garamond at 64pt. The composition leaves significant negative space below the title to reinforce the loneliness motif. The texture is a subtle film grain at 8% opacity.

3. **(Create)** Write the **Intent Layer** of a PROJECT.md for a creative project you have not started yet. Answer all five questions: *Who is this for? What is it for? What should the reader / viewer / listener feel? What argument does it make? What does it explicitly refuse to do?* Write in plain English, in your own voice. No Claude. Then read it to a classmate. Without telling them what you are making, ask them to describe what they think you are trying to make. If their description matches yours, the Intent Layer is doing its job. If it doesn't, the Intent Layer is not yet a Brutalist Intent Layer — revise until a stranger can recover the work from the document.
...
[^lewitt-1967]: LeWitt, Sol (June 1967). "Paragraphs on Conceptual Art." *Artforum* 5 (10): 79–83. Reproduction widely circulated; the essay's load-bearing sentence on *planning and decisions made beforehand* is on the first page of the original *Artforum* layout. The Notre Dame Hesburgh Libraries hold the original issue.

[^lewitt-1969]: LeWitt, Sol (January 1969). "Sentences on Conceptual Art." *0-9* (5). Reprinted in *Art-Language* 1 (1) (May 1969). The thirty-five aphorisms are widely reproduced; #10 (*Ideas alone can be works of art*) and #28 (*the process is carried out blindly*) are the load-bearing ones for the Brutalist analogy. Monoskop hosts a verified digital scan of the original 0-9 publication.

[^lewitt-tradition]: Yoko Ono's *Grapefruit* (Tokyo: Wunternaum Press, 1964) published instruction-art in sections — Music, Painting, Event, Poetry, Object — three years before LeWitt's *Artforum* essay. Adrian Piper, who studied with LeWitt and later became a Harvard analytic philosopher (Ph.D. 1981), articulated the intent-as-art position in explicitly philosophical terms beginning with *Catalysis* (1970–73). Hanne Darboven's numerical tables — "writing without describing" — are the closest art-historical analog to a CLAUDE.md. The instruction tradition is older, broader, and more diverse than the canonical LeWitt-centered story; the framework here is downstream of all of them.

---

**Bridge:** The reader has the full discipline. Act Three begins. Chapter 11 is the planning phase of their first complete build.
